...
class VaultPostLikeApiService {
    static let shared = VaultPostLikeApiService()
    var cancellable: AnyCancellable?
    
    func vaultPostLike(postId: String) -> AnyPublisher<Bool, AuthRegistrationAPIError> {
        guard let url = URL(string: FLAPIs.baseURL + FLAPIs.vaultPostLike) else {
            return Fail(error: .urlError).eraseToAnyPublisher()
        }
        
        do {
            let postLike = try JSONEncoder().encode(VaultPostLikeModel(post_uid: postId))
            ...
            print("PARAMS ---->", String(data: postLike, encoding: .utf8))
